      ...
      try {
        ...
        if (!moduleData) {
          setCModule(null);
            ...
            // ✨ FIX: Coerce the result to a strict boolean with !!
            const access =
              moduleData.isFree ||
            ...
            <div className="px-8">
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-4">
                        {cmodule.isFree ? (
                            <span className="bg-accent text-accent-foreground px-3 py-1 rounded-full text-sm font-semibold">{t('free')}</span>
                        ) : (
                        ...
                        <span className="bg-primary-foreground/20 text-primary-foreground px-3 py-1 rounded-full text-sm capitalize">{cmodule.level}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-6 text-sm">
                        <div className="flex items-center gap-2"><Clock className="h-5 w-5" /><span>{cmodule.duration}</span></div>
                        <div className="flex items-center gap-2"><BookOpen className="h-5 w-5" /><span>{cmodule.objectives.length} {t('objectives')}</span></div>
                        {hasAccess && (<div className="flex items-center gap-2 text-accent"><CheckCircle className="h-5 w-5" /><span>{t('accessGranted')}</span></div>)}
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className={`${showSidebar ? "lg:col-span-2" : "lg:col-span-3"} space-y-8`}>
                <div className="bg-card rounded-xl p-6 border border-border">
                  <h2 className="text-2xl font-bold text-foreground mb-4">{t('learningObjectives')}</h2>
                ...
                {hasAccess ? (
                  ...
                  <div className="bg-card rounded-xl p-12 border border-border text-center">
                    ...
                    <p className="text-muted-foreground mb-6">{t('contentLockedDescription')}</p>
                  </div>
                )}
              </div>
                ...
                <div className="lg:col-span-1">
                  <div className="bg-card rounded-xl p-6 border border-border sticky top-24">
                      ...
                      {!cmodule.isFree && (
                        <div className="text-center mb-4">
                          <span className="text-3xl font-bold text-foreground">
                            {new Intl.NumberFormat('fr-CM', { style: 'currency', currency: 'XAF' }).format(cmodule.price || 0)}
                          </span>
                    ...
                    <div className="mt-6 pt-6 border-t border-border space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between"><span>{t('duration')}</span><span className="text-foreground font-medium">{cmodule.duration}</span></div>
                      <div className="flex items-center justify-between"><span>{t('level')}</span><span className="text-foreground font-medium capitalize">{cmodule.level}</span></div>
                      <div className="flex items-center justify-between"><span>{t('access')}</span><span className="text-foreground font-medium">{t('lifetime')}</span></div>
                    </div>
                  </div>
                </div>
              )}
